...
test('renderBookPage escapes content and renders editable note fields', () => {
  ...
  assert.match(html, /data-sidebar-pane/);
  assert.match(html, /class="app-brand"/);
  assert.match(html, />Kindle Note</);
  assert.doesNotMatch(html, /class="app-mark"/);
  assert.match(html, /data-sidebar-toggle/);
  assert.match(html, /aria-label="收起侧边栏"/);
  assert.match(html, /title="Hide sidebar"/);
  assert.match(html, /class="sidebar-toggle-icon"/);
  assert.match(html, /class="library-pane"[^>]*>[\s\S]*data-sidebar-toggle[\s\S]*<\/aside>\n    <div class="sidebar-resizer"/);
  assert.doesNotMatch(html, /class="sidebar-resizer"[^>]*>[\s\S]*data-sidebar-toggle/);
  assert.match(html, /aria-label="调整侧边栏宽度"/);
  assert.match(html, /class="note-list-pane"/);
  assert.doesNotMatch(html, /<header class="list-header">\s*<div>[\s\S]*?<h1>/);
  assert.doesNotMatch(html, /<header class="list-header">[\s\S]*Unknown author/);
  ...
  assert.doesNotMatch(html, /data-note-mode="split"/);
  assert.doesNotMatch(html, />分栏</);
...
test('book page layout fills the viewport without decorative frames', () => {
  ...
  assert.match(APP_CSS, /--font-display: "Kaiti SC", "STKaiti"/);
  assert.match(APP_CSS, /--font-ui: -apple-system, BlinkMacSystemFont/);
  assert.match(APP_CSS, /\.reader-shell\s*{[^}]*grid-template-columns: var\(--library-width\) 8px var\(--note-list-width\) 8px minmax\(0, 1fr\)/s);
  assert.match(APP_CSS, /\.reader-shell\s*{[^}]*\n  height: 100vh;/s);
  assert.match(APP_CSS, /\.reader-shell\s*{[^}]*margin: 0/s);
  assert.match(APP_CSS, /\.library-pane\s*{[^}]*min-height: 0/s);
  assert.match(APP_CSS, /\.library-pane\s*{[^}]*overflow: auto/s);
  assert.match(APP_CSS, /\.note-list-pane\s*{[^}]*min-height: 0/s);
  assert.match(APP_CSS, /\.note-list-pane\s*{[^}]*overflow: auto/s);
  assert.match(APP_CSS, /\.detail-pane\s*{[^}]*min-height: 0/s);
  assert.match(APP_CSS, /\.detail-pane\s*{[^}]*overflow: auto/s);
  ...
  assert.doesNotMatch(APP_CSS, /\.sidebar-resizer:hover/);
  assert.doesNotMatch(APP_CSS, /\.note-list-resizer:hover/);
  assert.doesNotMatch(APP_CSS, /data-resizing-sidebar="true"[\s\S]*background:/);
  assert.doesNotMatch(APP_CSS, /data-resizing-note-list="true"[\s\S]*background:/);
  assert.doesNotMatch(APP_CSS, /\.section-label\s*{/);
  assert.match(APP_CSS, /\.library-pane\s*{[^}]*gap: 12px/s);
  assert.match(APP_CSS, /\.library-pane\s*{[^}]*padding: 18px 14px 18px/s);
  assert.match(APP_CSS, /\.note-list-pane\s*{[^}]*gap: 10px/s);
  assert.match(APP_CSS, /\.note-list-pane\s*{[^}]*padding: 20px 18px/s);
  assert.match(APP_CSS, /\.detail-pane\s*{[^}]*padding: 34px min\(4vw, 56px\) 34px/s);
  assert.match(APP_CSS, /\.detail-card\s*{[^}]*max-width: 900px/s);
  assert.match(APP_CSS, /\.detail-card\s*{[^}]*gap: 16px/s);
  ...
  assert.match(APP_CSS, /\.note-input\s*{[^}]*min-height: 220px/s);
  ...
  assert.match(APP_CSS, /\.note-preview\s*{[^}]*min-height: 220px/s);
  assert.doesNotMatch(APP_CSS, /data-note-view="split"/);
});
...
test('sidebar sections are collapsible and typographic hierarchy is explicit', () => {
  assert.match(APP_CSS, /--library-collapsed-width: 48px/);
  assert.match(APP_CSS, /\.reader-shell\[data-sidebar-collapsed="true"\]\s*{[^}]*grid-template-columns: var\(--library-collapsed-width\) 8px var\(--note-list-width\) 8px minmax\(0, 1fr\)/s);
  assert.match(APP_CSS, /\.reader-shell\[data-sidebar-collapsed="true"\]\s+\.library-pane\s*{/);
  assert.match(APP_CSS, /\.reader-shell\[data-sidebar-collapsed="true"\]\s+\.library-content\s*{/);
  assert.match(APP_CSS, /\.app-brand\s*{/);
  assert.match(APP_CSS, /\.app-brand\s*{[^}]*width: 100%/s);
  assert.doesNotMatch(APP_CSS, /\.app-mark\s*{/);
  assert.match(APP_CSS, /\.app-name\s*{[^}]*font-family: "Snell Roundhand", "Brush Script MT", cursive/s);
  assert.match(APP_CSS, /\.app-name\s*{[^}]*font-size: 34px/s);
  assert.match(APP_CSS, /\.reader-shell\[data-sidebar-collapsed="true"\]\s+\.app-brand\s*{[^}]*display: none/s);
  assert.doesNotMatch(APP_CSS, /\.sidebar-toggle\s*{[^}]*position: absolute/s);
  assert.match(APP_CSS, /\.sidebar-controls\s*{[^}]*justify-content: flex-end/s);
  assert.match(APP_CSS, /\.sidebar-toggle\s*{[^}]*width: 44px/s);
  assert.match(APP_CSS, /\.sidebar-toggle\s*{[^}]*background: rgba\(243, 244, 242, 0\.08\)/s);
  assert.match(APP_CSS, /\.sidebar-toggle:hover\s*{[^}]*background: rgba\(243, 244, 242, 0\.14\)/s);
  assert.match(APP_CSS, /\.sidebar-toggle-icon\s*{[^}]*width: 24px/s);
  assert.doesNotMatch(APP_CSS, /\.sidebar-toggle\s*{[^}]*box-shadow:/s);
  ...
  assert.match(APP_JS, /data-sidebar-toggle/);
  assert.match(APP_JS, /kindle-note:sidebar-collapsed/);
  assert.match(APP_JS, /function setSidebarCollapsed/);
...
test('app asset includes markdown note editing and export behavior', () => {
  ...
  assert.doesNotMatch(APP_JS, /noteView.*split|data-note-mode="split"|data-note-view="split"/);
  assert.match(APP_JS, /function autoSizeNoteInput/);
  assert.match(APP_JS, /Math\.max\(220, input\.scrollHeight \+ 2\)/);
