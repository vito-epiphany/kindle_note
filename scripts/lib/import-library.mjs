...
import { parseKindleSource } from './parse-kindle-note.mjs';
import { mergeBooks } from './merge-books.mjs';
...
export async function scanImportFolder(root = process.cwd()) {
  ...
  let books = existingBooks;
  if (allIncomingBooks.length > 0) {
    books = mergeBooks(existingBooks, allIncomingBooks).books;
  }
  ...
  return {
    ...
    importedFiles,
    parsedNotes: allIncomingBooks.reduce((sum, book) => sum + book.notes.length, 0),
    books
  };
}
